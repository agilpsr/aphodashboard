...
def render_dashboard(selected_key):
    ...
    # FIX: Better GPS column detection
    col_lat = None
    col_lon = None
    
    # Try multiple common latitude/longitude column patterns
    lat_patterns = ['_location_latitude', '_geopoint_latitude', 'latitude', '_latitude', 'lat']
    lon_patterns = ['_location_longitude', '_geopoint_longitude', 'longitude', '_longitude', 'lon', 'long']
    
    for pattern in lat_patterns:
        col_lat = col_map_lower.get(pattern)
        if col_lat:
            break
    
    for pattern in lon_patterns:
        col_lon = col_map_lower.get(pattern)
        if col_lon:
            break
    
    # If still not found, search for any column containing these keywords
    if not col_lat:
